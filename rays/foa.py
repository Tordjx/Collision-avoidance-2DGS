...
class MinimalRobot2D:
    ...
    def __init__(self, control_radius=0.3, control_point=np.array([0.0, 0.0])):
...
class ReactiveAvoidance:
    def __init__(self, control_radius=0.4):
        # Setup robot model
        self.robot = MinimalRobot2D(pose=ObjectPose(position=np.zeros(2), orientation=0.0))
    ...
    def compute(self, position, reference_velocity, obstacle_points):
        """Returns modulated velocity given obstacle points and reference velocity.
        
        Parameters:
        - position: np.array of shape (2,) – robot position in world frame
        ...
        self.avoider.update_laserscan(obstacle_points, in_robot_frame=True)
