...
class RaysThread:
    ...
    def run(self):
        while self.running:
            ...
            if inDepth is not None:
                ...
                frame = np.clip(frame, 0.15*1e3, 1e4)
    ...
    def _get_ray_points_local_frame(self, theta, depthFrame):
        ...
        cols = np.linspace(self.radius, self.frame_width - 1 -self.radius, self.N, dtype=int)
        ...
        for col in cols:
            ...
            min_val = np.median(patch[patch_mask])
            min_depths.append(float(min_val) / 1e3)
        print(min_depths)
        points = []
        for u, depth in zip(cols, min_depths):
            v = pixel_row
            z_cam = depth
            x_cam = (u - cx) * z_cam / fx
            y_cam = (v - cy) * z_cam / fy
