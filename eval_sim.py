...
csv_file = os.path.join("results", "evaluation_results.csv")
with open(csv_file, "w", newline="") as f:
    ...
    logger.info(f"Found {len(agent_files)} agents: {agent_files}")
    ...
    for agent_file in agent_files:
        ...
        model = CrossQ.load(agent_path, env=env, device=device)

        logger.info(f"Evaluating agent: {agent_name}")

        for ep in tqdm(range(100), desc=f"Agent {agent_name}"):
            obs, info = env.reset()
            ...
            # log to CSV
            writer.writerow([agent_name, ep + 1, steps])
            logger.info(f"{agent_name} | Episode {ep+1} length: {steps}")
